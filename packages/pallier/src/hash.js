export function FromhexToString(hexStr) {
...
export function hash(msg, option) {
  ...
  for (let i = 0; i < N; i++) {
    for (let j = 0; j < 16; j++) {
      ...
      if (M[i][j] < 0) binaryStr = (M[i][j] * -1).toString(2);
      else binaryStr = M[i][j].toString(2);
  ...
  for (let i = 0; i < N; i++) {
    ...
    for (let t = 0; t < 16; t++) {
      ...
      if (W[t] < 0) binaryStr = (W[t] * -1).toString(2);
      else binaryStr = W[t].toString(2);
