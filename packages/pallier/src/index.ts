...
export type GeneratedKeys = Awaited<ReturnType<typeof startGeneration>>;

export * from "./zkp";
export * from "./decrypt";

export function encrypt(message: bigint, N: bigint, r?: bigint) {
  let random = r || Gen_Coprime(N);
  let ciphertext = ((PowerMod(generator(N), message, N * N) as bigint) *
    PowerMod(random, N, N * N)) as bigint;
  return ciphertext % (N * N);
}

export function combineCiphertext(existing: bigint, c: bigint) {
  return existing * c;
}
...
export async function startGeneration(
  ...
  const thisPartyNumber = allParties.find(
    (p) => p.party.partyId === userId
  )!.partyNumber;

  console.log({ thisPartyNumber });

  const [N, p, q] = await RSA_ModulusGeneration(
    allParties,
    otherParties,
    thisPartyNumber,
    keyLength,
  ...
  otherParties.forEach((party) =>
    party.send(JSON.stringify({ pqSum: (p + q).toString() }))
  );

  //get pqSums of other parties
  const pqSums = await firstValueFrom(
    zip(
      ...otherParties.map((party) =>
        party.bufferedMessage$.pipe(
          map((e: any) => BigInt(JSON.parse(e).pqSum))
        )
      )
    )
  );
  ...
  otherParties.forEach((party) =>
    party.send(JSON.stringify({ theta: myTheta.toString() }))
  );

  //get thetas of other parties
  const otherThetas = await firstValueFrom(
    zip(
      ...otherParties.map((party) =>
        party.bufferedMessage$.pipe(
          map((e: any) => BigInt(JSON.parse(e).theta))
        )
      )
    )
  );
  ...
  const ourShare =
    r_delta + evaluate_polynomial(coeff, BigInt(thisPartyNumber));
  ...
  const other_r_deltas = await lastValueFrom(
    zip(
      ...otherParties.map((party) =>
        party.bufferedMessage$.pipe(
          map((e: any) => BigInt(JSON.parse(e).r_delta))
        )
      )
    )
  );
  ...
  if (!firstParty) {
    ...
    otherParties.forEach((party) =>
      party.send(JSON.stringify({ vk: vk.toString() }))
    );
  } else {
    vk = await firstValueFrom(
      firstParty.bufferedMessage$.pipe(map((e) => BigInt(JSON.parse(e).vk)))
    );
  }
