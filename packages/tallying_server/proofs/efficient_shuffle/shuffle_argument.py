...
from proofs.efficient_shuffle.multi_exponantiation_argument import (
    MultiExponantiation,
)
from proofs.efficient_shuffle.product_argument import ProductArgument
...
class ShuffleArgument:
    ...
    def __init__(
        ...
        self.shuffled_ciphertexts = shuffled_ciphertexts
        ...
        self.exp_permutation_comm = [
            com_pk.commit(
                exp_challenge_pem[i], randomizers_exp_permutation_comm[i])[0]
            for i in range(self.m)
        ]
        ...
        commitment_D = [
            (self.permutation_comm[i] ** self.challenge2) *
            self.exp_permutation_comm[i]
            for i in range(self.m)
        ]
    ...
    def __repr__(self):
        ...
        return f' PROOOOOO O SHUFFLE'

    def __str__(self):
        return F'yahan shuffle proof print hona hai'
    ...
    def verify(self, com_pk, pk, ciphertexts, shuffled_ciphertexts):
        ...
        check2 = all(
            [
                com_pk.group.check_point(
                    self.exp_permutation_comm[i].commitment)
                for i in range(self.m)
            ]
        )
        ...
        check3 = self.product_argument_proof.verify(
            com_pk, commitment_A, product)
    ...
    def prepare_ctxts(ctxts, m, election_key):
        ...
        if type(ctxts[0]) == elgamal.Ciphertext:
            ...
            encrypted_counter = election_key.encrypt(
                counter * group.generator())
