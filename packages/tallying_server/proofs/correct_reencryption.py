...
from primitives.ballot_structure import VoteVector, PointVector
...
class ReencryptionProof:
    ...
    def __init__(self, kp_tally, pk_vote, tokens, vote, max_tag=None):
        ...
        if self.dummy:
            ...
            self.reencrypted_vote = pk_vote.reencrypt(
                vote, randomizer_reencryption)
        ...
        if last_token_decryption == 0 * self.group_tally.generator():
            ...
            self.proof_real = prover_real.create_proof(
                [commitment_pk, commitment_token,
                    commitment_vote1, commitment_vote2],
                self.true_challenge,
            )
    ...
    def verify(self, pk_tally, pk_vote, tokens, vote):
        ...
        verify_dummy = self.proof_dummy.verify(
            pk_tally, pk_vote, self.added_token, self.reencrypted_vote, len(
                tokens)
        )
...
class RealVoterReencryptionProver:
    ...
    def create_proof(self, commitments, challenge=None):
        ...
        return RealVoterReencryptionProof(
            commitments, [response_sk,
                          response_randomizer], challenge=proof_challenge
        )
    ...
    def simulate(self):
        ...
        commitment_vote1 = (
            PointVector(
                [fake_response_randomizer * self.pk_vote.generator] *
                self.nr_candidates
            )
            / (
...
class RealVoterReencryptionProof:
    ...
    def verify(self, pk_tally, pk_vote, token, vote, reencrypted_vote):
        ...
        check_reencryption1 = (
            PointVector([self.response_randomizer *
                        pk_vote.generator] * nr_candidates)
            == self.commitment_vote1
