...
import primitives.pedersen_commitment as commitment
import primitives.elgamal as elgamal

from primitives.ballot_structure import BallotBundle, VoteVector
from dummies.dummies import compute_cover
from proofs.efficient_shuffle.shuffle_argument import ShuffleArgument
from proofs.correct_reencryption import ReencryptionProof
from proofs.correct_decryption import CorrectDecryption
...
class Filter:
    ...
    def __init__(self, kp, voting_key, list_ctxts, m, counter_lookup_table, votes_lookup_table):
        """
        Perform and generate proof of tally
        """
        self.pk = kp.pk
        self.election_key = voting_key
        print(type(voting_key))
        self.group = kp.group
        self.generator = self.group.generator()
        self.order = self.group.order()
        self.m = m
        #self.vid_lookup_table = vid_lookup_table
        self.nr_candidates = list_ctxts[0].vote.length
        self.votes_lookup_table = votes_lookup_table

        self.counter_lookup_table = counter_lookup_table
        self.highest_counters =[]
        # Step 2: We begin by generating the dummy votes
        self.initial_ctxts = list_ctxts
        print("Initial Ballots")
       # print(list_ctxts)

        self.dummies = self.generate_dummies(
            kp, voting_key, self.initial_ctxts)

        print("Generating dummies", self.dummies)

        print(" \n Length of dummies", len(self.dummies))

        self.overhead = len(self.dummies)

        # --------------------adds the dummy votes in total votes
        ...
        print("Step 3: verifiable shuffle of stripped ciphertexts")
        ...
        print("\n proof_of_shuffle", self.proof_of_shuffle.verify(self.com_pk,
                                                                  self.pk,
                                                                  self.ctxts,
                                                                  self.shuffled_ctxts,
                                                                  ))
        ...
        print(" \n Step 3 permutation ", self.permutation)
        print(" \n Step4 randomizer ", reshaped_randomizers)
        ...
        print("Step 5: Group ballots per vid and selecting the highest index, and reencrypting ballots")
        ...
        df = pd.DataFrame(
            stripped_ballots_dec_vid_index, columns=[
                "vid", "index", "tag", "entry_nr"]
        )
        #print("dataframe\n", df)

        # We select the entries with the maximum index. The entry_number is the position in self.reshaped_shuffled_ctxts

        indexes = df.groupby(["vid"], sort=False)[
            "index"].transform(max) == df["index"]
        ...
        print("=================================================== \n ")
        # output of renencrypted votes is being generated from remcrypt and proof function
        # print("rencrypted votes", self.reencrypted_votes)

        # Step 6: randomize and shuffle selected votes

        print("Step 6: randomize and shuffle selected votes")

        #assert self.nr_candidates == 1
        # selecting the highest index vote from the ballot
        self.selected_votes = [v.ballot[0] for v in self.reencrypted_votes]
        print("test hier", self.selected_votes)

        # We shuffle the reencrypted_votes once more so we can drop the ones corresponding to dummy ballots
        (
        ...
        print("CTXTS \n", self.selected_votes_padded,  " FINAL RANDOMIZER \n", final_randomizers, "LENGTH OF RANDOMIZER =====", len(final_randomizers), "SHUFFLED CTXTS \n",
               self.selected_votes_shuffled, "PERMUTATION \n", permutation, "N=====\n", n)

        self.final_length_total = len(self.ctxts)
        # print("\n Final length total", self.final_length_total)
        ...
        print("\n proof_of_Final_shuffle", self.final_proof_of_shuffle.verify(self.final_com_pk,
                                                                              self.pk,
                                                                              self.selected_votes_padded,
                                                                              self.selected_votes_shuffled,
                                                                              ))
        ...
        # selected votes padded are the votes with the dummy votes
        ...
        self.final_permutation = [
            p for sublist in permutation for p in sublist]
        ...
        print("Step 7: open dummy ballots that should be ignored in the next phase")

        self.actual_votes = []
        self.revealed_dummy_indices = range(
            self.nr_ballot_groups - self.nr_voters)
        ...
        # print(self.revealed_dummy_indices)
       # Removing dummies and printing
        print("With DUMMIES", "\n")

        for i in self.selected_votes_shuffled:
            for j in i:
                print(j.decrypt(kp.sk))
        print("Without DUMMIES", "\n")

        for i in self.selected_votes_shuffled:
            for j in i:
                print( "raw j ", j)
                if (j.decrypt(kp.sk) in self.votes_lookup_table):
                    self.actual_votes.append(
                        votes_lookup_table[j.decrypt(kp.sk)])
                    print(votes_lookup_table[j.decrypt(kp.sk)])
    ...
    def verify(self):
        ...
        zero_vote = VoteVector(
            [self.election_key.encrypt(
                self.group.infinite(), 0)] * self.nr_candidates
        )
        dummy_tag = elgamal.Ciphertext(
            self.group.infinite(), self.group.generator())
        ...
        for index, entry in enumerate(self.decrypted_vid_index):
            proof_decryptions.append(
                entry[1].verify(
                    self.reshaped_shuffled_ctxts[index].vid, entry[0])
            )
            proof_decryptions.append(
                entry[3].verify(
                    self.reshaped_shuffled_ctxts[index].index, entry[2])
            )
        ...
        for index, proof in enumerate(self.reencryption_proofs):
            proof_reencryptions.append(
                proof[1].verify(
                    self.pk,
                    self.pk,
                    getattr(self.tags, str(
                        self.decrypted_vid_index[proof[0]][0])),
    ...
    def reencrypt_and_proof(self, kp, ctxts, entry_numbers, tags):
        ...
        for i in entry_numbers:
            ...
            proof = ReencryptionProof(
                kp, kp.pk, tags_vid, ballot.vote, max_tag=max_tag)
            ...
            # ----
            print("rencrypted vote", proof.reencrypted_vote)
            print("proof of reencrypted vote", proof.verify(proof.pk_tally,
                                                            proof.pk_vote, proof.tokens, proof.vote))
    ...
    def generate_dummies(self, kp, voting_key, list_ctxts):
        ...
        for ballot in list_ctxts:
            ...
            print("voted", vote.ballot[0].decrypt(sk))
            decryptedVote = hex(self.votes_lookup_table[vote.ballot[0].decrypt(sk)])
            decryptedCounter = self.counter_lookup_table[counter]
            # print("counter from lookup", )
            # print("vote from lookup", )
            self.counter_vote_list.append ({
        ...
        self.nr_voters = nr_ocurrences.shape[0]  # (m,n)
        nr_ballots = len(list_ctxts)

        print("number of voters ========", self.nr_voters,
              "number of votes ====", nr_ballots)
        ...
        min_counter = df.groupby(["vid"], sort=False)[
            "min_counter"].min().reset_index()
    ...
    def create_single_dummy(self, pk, vid=None, counter=None, dummy=True):
        ...
        real_tag = elgamal.Ciphertext(
            ts_key.group.infinite(), ts_key.group.infinite())
    ...
    def decrypt_and_proof(self, kp):
        ...
        for i, ballot in enumerate(self.reshaped_shuffled_ctxts):
            ...
            # if its a dummy it will just print dummy otherwise orginal value
            if vid in self.counter_lookup_table:
                print(" ORIGINAL VALUE", self.counter_lookup_table[vid])
            else:
                print("dummy")

            print("vid", vid, " proof", proof_vid.verify(
                proof_vid.ciphertext, proof_vid.plaintext))

            index = self.decrypted_counter[self.permutation[i]]
            proof_index = CorrectDecryption(ballot.index, index, kp)
            

            print("index", index, "proof", proof_index.verify(
                proof_index.ciphertext, proof_index.plaintext), "\n")
            # getting the highest counters
            print ("raw index")
            print(proof_index.plaintext)
            self.decrypted_vid_index.append(
                [vid, proof_vid, index, proof_index])

            stripped_ballots_dec_vid_index.append(
                [str(vid), self.counter_lookup_table[index], ballot.tag, i])

            # stripped_ballots_dec_vid_index.append(
            #     [self.vid_lookup_table[vid], self.counter_lookup_table[index], ballot.tag, i])

        # print(" 3215 decrypted Voterids \n", stripped_ballots_dec_vid_index)
        # print("decrypted Voterids Proof \n", proof_vid)

        print("decrypted Counter \n", self.decrypted_counter)
        #print("decrypted counter Proof \n", proof_index)
    ...
    def perform_shuffle(self, input_ctxts):
        ...
        print("cx2", randomizers)
        permutation = np.random.permutation(length_total).tolist()
        

        shuffled_ctxts = [
            self.pk.reencrypt(ctxts[permuted_index],
                              ephemeral_key=randomizers[index])
        ...
        print("PRINTING SHUFFLE RETURNS")

        # print("CTXTS \n", ctxts, "RANDOMIZER \n", randomizers, "LENGTH OF RANDOMIZER =====", len(randomizers), "SHUFFLED CTXTS \n",
        #       shuffled_ctxts, "PERMUTATION \n", permutation, "N=====\n", n)
    ...
    def prepare_ctxts(self, ctxts, m, election_key):
        ...
        """
        import math
        print("prepare_ctxts", m)
        print("len ctxts", len(ctxts))
        '''
        if len(ctxts) < m:
            print("errrrror")
            raise ValueError(
        ...
        if type(ctxts[0]) == BallotBundle:
            ...
            for _ in range(m * n - len(ctxts)):
                dummy, vid, counter = self.create_single_dummy(
                    election_key, dummy=True)
