...
import "../client/node_modules/elliptic-curve-solidity/contracts/EllipticCurve.sol";


/**
 * @title Verifiable Random Functions (VRF)
 * @notice Library verifying VRF proofs using the `Secp256R1` curve and the `SHA256` hash function.
 * @dev This library follows the algorithms described in [VRF-draft-06](https://datatracker.ietf.org/doc/html/draft-irtf-cfrg-vrf-06) and [RFC6979](https://tools.ietf.org/html/rfc6979).
 * It supports the _SECP256R1_SHA256_TAI_ cipher suite, i.e. the aforementioned algorithms using `SHA256` and the `Secp256R1` curve.
...
library VRF_P256 {
  ...
  function gammaToHash(uint256 _gammaX, uint256 _gammaY) internal pure returns (bytes32) {
    bytes memory c = abi.encodePacked(
      // Cipher suite code (SECP256R1-SHA256-TAI is 0x01)
      uint8(0x01),
      // 0x03
      uint8(0x03),
  ...
  function verify(uint256[2] memory _publicKey, uint256[4] memory _proof, bytes memory _message) internal pure returns (bool) {
    ...
    return uint128(derivedC) == uint128(_proof[2]);
  }
  ...
  internal pure returns (bool)
  {
    ...
    // V = s*H - c*Gamma
    /* Ecrecover work with secp256k1 signature!!!
