...
export async function RSA_ModulusGeneration(
  allParties: any[],
  otherParties: Party[],
  thisPartyNumber: number,
  keyLength: bigint,
  ...
  if (!firstParty) {
    ...
    const { PP: data } = JSON.parse(
      await firstValueFrom(firstParty.bufferedMessage$)
    );
    PP = BigInt(data);
  }
  ...
  while (!generation_done) {
    ...
    let selfTuple: ReturnType<typeof compute_tuple>;
    ...
    for (const party of allParties) {
      ...
      if (party.type == "other") {
        party.party.send(
          JSON.stringify({ tuple: t.map((el) => el.toString()) })
        );
      } else {
        selfTuple = t;
      }
    ...
    const tuples = await firstValueFrom(
      zip(
        ...otherParties.map((p) =>
          p.bufferedMessage$.pipe(
            map((message: any) =>
              (JSON.parse(message).tuple as string[]).map((s: string) =>
                BigInt(s)
              )
            )
          )
    ...
    const NVals = await firstValueFrom(
      zip(
        ...otherParties.map((p) =>
          p.bufferedMessage$.pipe(
            map((e) => JSON.parse(e)),
            map((e: any) => BigInt(e.Ni))
          )
        )
      )
    );
    ...
    let Qi_inv: bigint;
    if (firstParty) {
      const gg = await firstValueFrom(
        firstParty.bufferedMessage$.pipe(map((e) => BigInt(JSON.parse(e).gg)))
      );
      ...
      otherParties.forEach((party) =>
        party.send(JSON.stringify({ gg: gg.toString() }))
      );
    ...
    otherParties.forEach((party) =>
      party.send(JSON.stringify({ Qi: Qi_inv.toString() }))
    );

    //get otherQis
    const otherQs = await firstValueFrom(
      zip(
        ...otherParties.map((party) =>
          party.bufferedMessage$.pipe(map((e: any) => BigInt(JSON.parse(e).Qi)))
        )
      )
    );

    const m = otherQs.reduce(
      (acc: bigint, q_inv: bigint) => acc * q_inv,
      Qi_inv
    );
