...
contract Evoting {
    ...
    mapping(uint256 => mapping(uint256 => Utils2.Vote[])) public votes_per_ballot_paper_type2_per_candidates; // ballotID => candidate index => votes[]
    mapping(uint256 => bool) public is_valid_ballot; // Check if ballot was not deleted(false).
    mapping(uint256 => Utils2.Vote[]) public votes_per_ballot_paper_type1; // Map votes to the corresponding ballot paper.
    mapping(address => Utils2.Vote[]) public votes_per_voters;
    ...
    mapping(uint256 => Utils2.Vote[]) public finalVotes;
    ...
    *@param _ballot_ids Array containing the ids of the ballot paper to remove.
    ...
    function delete_ballots(uint256[] memory _ballot_ids) private {
        for (uint256 i = 0; i < _ballot_ids.length; i++) {
            require(_ballot_ids[i] < ballot_papers.length, "Vote not found.");
    ...
    function update_ballot(Utils2.BallotDetails[] memory _ballots) private {
    ...
    function update_vote(Utils2.VoteDetails memory _details, Utils2.BallotDetails[] memory _to_update, Utils2.BallotPaper[] memory _to_add, uint256[] memory _to_delete) external onlyAdmin {
    ...
    function cast_vote(Utils2.Vote[][] memory _votes) external {
    ...
    function get_votes() external view returns(Utils2.Vote[][] memory) {
        uint256 length = ballot_papers.length;
        Utils2.Vote[][] memory _votes = new Utils2.Vote[][](length);
        for (uint256 i = 0; i < length; i++) {
    ...
    function cancel_vote() external onlyAdmin {
    ...
    function get_vote_details() external view 
