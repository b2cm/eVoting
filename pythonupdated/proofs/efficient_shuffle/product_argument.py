...
import primitives.pedersen_commitment as com
import proofs.efficient_shuffle.multi_exponantiation_argument as mult_exp
...
class ProductArgument:
    ...
    def verify(self, com_pk, commitment, product):
        ...
        check2 = self.hadamard.verify(
            com_pk, commitment, self.commitment_products)
        check3 = self.single_value.verify(
            com_pk, self.commitment_products, product)
...
class SingleValueProdArg:
    ...
    def __init__(self, com_pk, commitment, product, committed_values, randomizer):
        ...
        self.announcement_one, _ = com_pk.commit(
            d_randoms, commitment_rand_one)
        ...
        self.response_committed_values = [
            (self.challenge *
             committed_values[i] + d_randoms[i]).mod(self.order)
            for i in range(self.n)
        ]
    ...
    def verify(self, com_pk, commitment, product):
        ...
        value_to_commit_check5 = [
            (
                ...
                - self.response_product[i] *
                self.response_committed_values[i + 1]
            ).mod(self.order)
            for i in range(self.n - 1)
        ]
        check5 = (
...
class ZeroArgument:
    ...
    def __init__(
        ...
        for k in range(2 * self.m + 1):
            ...
            for i in range(self.m + 1):
                ...
                diagonal += (
                    self.bilinear_map(
                        A[i], B[j], self.bilinear_const, self.order)
                ).mod(self.order)
            diagonals.append(diagonal)

        commitment_rand_diagonals = [self.order.random()
                                     for _ in range(2 * self.m + 1)]
        commitment_rand_diagonals[self.m + 1] = 0

        self.announcement_diagonals = [
            com_pk.commit_reduced(
                [diagonals[i]], 1, commitment_rand_diagonals[i])[0]
        ...
        self.response_randomizer_diagonals = modular_sum(
            [
                (
                    self.challenge.mod_pow(
                        i, self.order) * commitment_rand_diagonals[i]
                ).mod(self.order)
                for i in range(self.m * 2 + 1)
            ],
            self.order,
    ...
    def verify(self, com_pk, commitment_A, commitment_B):
        ...
        check3 = all(
            [
                com_pk.group.check_point(
                    self.announcement_diagonals[i].commitment)
                for i in range(self.m * 2 + 1)
            ]
        )
        ...
        exponents_5 = [self.challenge.mod_pow(
            i, self.order) for i in range(self.m + 1)]
        check5 = (
            mult_exp.MultiExponantiation.comm_weighted_sum(
                commitment_A, exponents_5)
            == com_pk.commit_reduced(
...
class HadamardProductArgument:
    ...
    def __init__(
        ...
        self.challenge_bilinear = compute_challenge(
            transcript_bilinear, self.order)
    ...
    def verify(self, com_pk, commitment_A, commitment_b):
        ...
        exponents = [self.challenge.mod_pow(
            i, self.order) for i in range(self.m - 1)]
