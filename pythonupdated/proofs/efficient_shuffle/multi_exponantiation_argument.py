...
class MultiExponantiation:
    ...
    def __init__(
        ...
        announcementB_randomisers = [self.order.random()
        ...
        for k in range(2 * self.m):
            ...
            for i in range(self.m):
                ...
                diagonal *= self.ctxt_weighted_sum(
                    ciphertexts[i], exponents[j])
        ...
        self.responseA = [
            sum([exponents[j][i] * challenge_powers[j]
    ...
    def verify(
        ...
        if self.type == elgamal.Ciphertext:
            ...
            check3 = all(
                [
                    pk.group.check_point(
                        self.announcement_reencryption[i].vid.c1)
        ...
        check4 = self.announcementB[self.m] == com_pk.commit_reduced([0], 1, 0)[
            0]
        check5 = (
        ...
        check6 = (
            product_A == com_pk.commit(
                self.responseA, self.responseA_randomizers)[0]
        )

        exponents_product_B = [
            self.challenge.mod_pow(i, self.order) for i in range(self.m * 2)
        ]
        product_B = self.comm_weighted_sum(
            self.announcementB, exponents_product_B)
        check7 = (
            product_B
    ...
    def ctxt_weighted_sum(list_ctxts, weights):
        ...
        if ctxt_type == elgamal.Ciphertext:
            ...
            return BallotBundle(
                ...
                elgamal.Ciphertext(
                    group.wsum(weights, c1s_index), group.wsum(
                        weights, c2s_index)
                ),
                elgamal.Ciphertext(
